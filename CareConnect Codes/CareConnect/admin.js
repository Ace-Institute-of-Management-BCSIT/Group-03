...
async function loadPendingNgos() {
    ...
    try {
        ...
        if (!data.ngos.length) {
            list.innerHTML = '<p class="admin-empty">No pending applications right now.</p>';
            return;
        }
        ...
        list.innerHTML = data.ngos.map(n => `
            <div class="admin-card">
                ...
                <div class="admin-card-meta">
                    ...
                    <b>Focus:</b> ${n.focus_area} &middot; <b>District:</b> ${n.district}<br>
                    <b>Reg. No:</b> ${n.reg_number}<br>
                    <b>Contact:</b> ${n.contact_person} &middot; ${n.email}${n.phone ? ' · ' + n.phone : ''}<br>
        ...
        console.error(err);
        list.innerHTML = '<p class="admin-empty">Could not load pending NGOs.</p>';
    }
}
...
async function approveNgo(ngoId) {
    ...
    try {
        const res = await fetch('api/admin_actions.php', {
            ...
            body: JSON.stringify({ action: 'approve_ngo', admin_id: adminId(), ngo_id: ngoId })
        });
        const data = await res.json();
...
async function rejectNgo(ngoId) {
    if (!confirm('Reject this NGO application? This cannot be undone.')) return;
    showToast('⏳ Rejecting...');
    try {
        const res = await fetch('api/admin_actions.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'reject_ngo', admin_id: adminId(), ngo_id: ngoId })
        });
        const data = await res.json();
        showToast(data.success ? '✅ Application rejected.' : '⚠️ ' + data.message);
        loadPendingNgos();
    } catch (err) {
        console.error(err);
        showToast('⚠️ Server error');
...
async function loadVerifiedNgos() {
    ...
    try {
        ...
        list.innerHTML = data.ngos.map(n => `
            <div class="admin-card">
                ...
                <div class="admin-card-meta">
                    ...
                    <b>Campaigns posted:</b> ${n.campaign_count}
                </div>
            </div>
        `).join('');
        ...
        console.error(err);
        list.innerHTML = '<p class="admin-empty">Could not load NGOs.</p>';
    }
}
...
async function loadDonations() {
    ...
    try {
        ...
        if (!data.success) {
            cashBody.innerHTML = `<tr><td colspan="6" class="admin-empty">${data.message}</td></tr>`;
            return;
        }
        ...
        `).join('') : '<tr><td colspan="6" class="admin-empty">No cash donations yet.</td></tr>';
        ...
        `).join('') : '<tr><td colspan="5" class="admin-empty">No item donations yet.</td></tr>';

    } catch (err) {
        console.error(err);
        cashBody.innerHTML = '<tr><td colspan="6" class="admin-empty">Could not load donations.</td></tr>';
    }
}
...
async function deleteUser(userId) {
    if (!confirm('Remove this user permanently? This cannot be undone.')) return;
    showToast('⏳ Removing...');
    try {
        const res = await fetch('api/admin_actions.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'delete_user', admin_id: adminId(), user_id: userId })
        });
        const data = await res.json();
