...
/* -------------------------
   CASH DONATION — QR PAYMENT FLOW
-------------------------- */
let pendingDonation = null; // { amount, transaction_id }
...
async function handleDonate() {
    ...
    showToast('⏳ Generating payment QR...');

    try {
        const res = await fetch('api/qr_generate.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ user_id: session.id, cause_id: currentCauseId, amount })
        });
        const data = await res.json();

        if (!data.success) {
            showToast('⚠️ ' + (data.message || 'Could not start payment'));
            return;
        }

        pendingDonation = { amount, transaction_id: data.reference };
        openQrModal(data.qr_payload, data.reference, amount);
...
function openQrModal(payload, reference, amount) {
    ...
    document.getElementById('qrAmountLabel').textContent = `NPR ${amount.toLocaleString()}`;
    document.getElementById('qrRefLabel').textContent = reference;

    document.getElementById('qr-payment-modal')?.classList.add('show');
    document.body.style.overflow = 'hidden';
}

function closeQrModal() {
    document.getElementById('qr-payment-modal')?.classList.remove('show');
    document.body.style.overflow = '';
    pendingDonation = null;
}

async function confirmQrPayment() {
    if (!pendingDonation) { closeQrModal(); return; }
    ...
    try {
        ...
        if (data.success) {
            showToast(`🎉 Thank you! NPR ${pendingDonation.amount.toLocaleString()} donated.`);
            closeQrModal();
            renderRecentDonors();
            loadDynamicCauses();
        } else {
