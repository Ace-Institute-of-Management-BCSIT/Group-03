...
async function loadDynamicOpportunities() {
    ...
    try {
        ...
        container.innerHTML = needVolunteers.map(c => {
            const isUrgent = c.volunteers_needed <= 5;
            const orgName  = c.ngo_name || 'NGO';
            return `
            <div class="opp-card" data-cat="${(c.category || '').toLowerCase()}">
                <div class="opp-card-top">
                    <div class="opp-header">
                        <div class="opp-org">
                            <div class="org-logo" style="background:${colorForName(orgName)};">${orgName[0].toUpperCase()}</div>
                            <div><div class="org-info-name">${orgName}</div><div class="org-info-type">NGO · Verified</div></div>
                        </div>
                        ${isUrgent ? '<span class="opp-urgent">🔥 Urgent</span>' : ''}
                    </div>
                    <div class="opp-title">${c.title}</div>
                    <div class="opp-desc">${c.description || ''}</div>
                    <div class="opp-tags">${c.category ? `<span class="opp-tag">${c.category}</span>` : ''}${c.district ? `<span class="opp-tag">${c.district}</span>` : ''}<span class="opp-tag">Volunteers needed</span></div>
                    <div class="opp-meta">
                        <div class="opp-meta-item"><i class="fa-solid fa-location-dot"></i> ${c.district || 'Nepal'}</div>
                        <div class="opp-meta-item"><i class="fa-solid fa-calendar"></i> Ongoing</div>
                        <div class="opp-meta-item"><i class="fa-solid fa-hand-holding-heart"></i> NPR ${Math.round(c.raised_amount).toLocaleString()} raised</div>
                        <div class="opp-meta-item"><i class="fa-solid fa-bullseye"></i> Goal NPR ${Math.round(c.goal_amount).toLocaleString()}</div>
                    </div>
                </div>
                <div class="opp-card-bottom">
                    <div class="opp-spots"><strong>${c.volunteers_needed}</strong> spots left</div>
